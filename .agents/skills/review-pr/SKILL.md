...
```bash
gh pr view <PR番号> --json title,body,headRefName,baseRefName,files,commits,labels
gh pr diff <PR番号>
```
...
> **本警告の発火閾値 (≥1 件) と、Step 5a 起動条件 2 の閾値 (≥2 件) は別である。** 過去 merged PR が
> 1 本のときは「M5 警告は出るが Codex 起動条件 2 は不成立」という中間状態になる。警告行を見て
> 条件 2 成立と読まないこと (同じシグナルを 2 つの gate が別閾値で使っている)。
...
gh pr view "<PR番号>" --json createdAt,baseRefOid,headRefOid,changedFiles,files,mergeStateStatus
...
gh pr view "<PR番号>" --json closingIssuesReferences,body
...
**まず `enforce-acceptance-criteria <PR番号>` を呼び出し**、Iron Law (全項目の逐条検証) によるゲートを通過させる。このスキルが未達と判定した場合、ここで処理を終了し修正依頼フローへ進む。
...
>
> `AGENTS.md` §「destructive write boundary audit checklist」 の「抽象語の観点は具体的な欠陥クラスへの検出力を持たない。検出力は具体列挙にのみ宿る」と同じ理由で、gate は具体条件で書く。
...
- 重点観点: logic correctness / architecture (`AGENTS.md` §モジュール構成、`docs/design-overview.md`) / security (subprocess 呼び出し / 外部入力 / 認証情報) / code smell
...
- 関連する既存 doc (`AGENTS.md`, `docs/cli-spec.md`, `docs/design-overview.md`, `docs/l2-workflow.md` 等) との矛盾がないか
- doc が言及するテスト / CLI 出力サンプルが実装と一致しているか

**コード / テスト変更 PR の場合**:

- 関連ドキュメント (`docs/cli-spec.md`, `docs/design-overview.md`, `README.md`, `AGENTS.md` 等) が更新されているか
- **特に AGENTS.md / docs に「追加予定」「今後実装」等の予告記述があり、本 PR がその実装に該当する場合、予告文を実装済み記述に更新すること。更新漏れは Step 6 で修正依頼対象**
...
> **「Fable にレビューさせた」は Codex review 省略の口実にしない** (`AGENTS.md` §「Fable と Codex の棲み分け」)。両者は併存レイヤーであり、invariant / 不可逆操作に関わる変更は**両方**にかける。
...
以下のいずれかを満たす PR で Codex review を併走させる (人手 trigger or skill 内 auto)。agent 実行は tier 1 = `codex` CLI 直接呼び出し `codex review --base develop-X.Y.Z` の Bash 実行 (§Codex 運用、`docs/l2-workflow.md` §Step 5 の invocation path (3-tier、#795) 参照):
...
(`AGENTS.md` §destructive write boundary audit checklist) は、レビュアの**起動条件の書き方**にも
そのまま当てはまる。
...
> **判定は short-circuit してよいが、記録は全数走査を要する。** 条件 3 の**成立判定**は
> 「1 ファイルでも該当」なので 1 件見つけた時点で確定できる。しかし**起動記録の `該当 <J> 件`**
> は touched files 全件を表と突き合わせた数でなければならない。起動が確定した時点で数え止めると
> J が過少になり、後で表を編集したときに過去の判定を再現できなくなる (記録に列挙を義務づけている
> 理由がこれ)。**走査量が判定と記録で違う**ことをここで明示しておく。
>
...
> `Codex review 起動: 対象 (理由: 条件1 touched <N> file / <M> lines・条件2 再発 root cause <K> 件・条件3 core 変更対象ファイル 該当 <J> 件 [<該当ファイル列挙>] → 成立: 条件 <成立した条件番号を全て>)`
>
> `Codex review 起動: 非対象 (理由: 条件1 touched <N> file / <M> lines・条件2 再発 root cause <K> 件・条件3 core 変更対象ファイル 該当 0)`

**実測値の書式は両分岐で共通**とする (#993 / #994)。どちらの分岐でも **3 条件を 1 つずつ名指しし、
それぞれに実測値を付ける**。「`non-L1-core`」のような抽象語 1 語では、事後に「その判定が
正しかったか」を検証できない。

- **条件 1**: touched file 数と diff 行数を両方書く
- **条件 2**: 再発 root cause の件数 (= Step 1.1 の M5 警告件数)
- **条件 3**: §「core 変更対象ファイル」の表に対する**該当件数**。0 件なら `該当 0`、
  **1 件以上なら該当ファイルを角括弧で列挙する** (どのファイルで成立したかが分からないと、
  ...
  §「変更ファイル一覧が取れないときの挙動 (fail-closed、実装の契約)」の規定に従う。部分リストで数えた `該当 <J> 件` は
...
Codex review が exit 0 で完了したら、**finding を stdout から拾う**。`codex` CLI は非対話で stdout に直接結果を出して終了する (旧 companion script の job 追跡機構 `status` / `result` / job id は廃止済)。契約の正は [`docs/l2-workflow.md` §「Codex 出力の読み取り」](../../../docs/l2-workflow.md) で、本 step はその適用にあたる。

1. review を実行した worktree の cwd で `codex review --base <base> "<focus>"` を実行し、**stdout をそのまま finding の入力にする**:

   ```bash
   cd "<review を実行した worktree の絶対パス>"   # cwd は turn 境界で main repo へドリフトしうる。明示する
   codex review --base <base> "<focus>"
   #   → finding は stdout に出る。exit code 0 = 成功 / 非ゼロ = 失敗 (§Codex fallback)
   ```

   > **`cd` を省略しない。** 本 repo では turn 境界や background task の後に Bash の cwd が
...
|---|---|---|---|
| 1 | <条件 1> | `path/to/file.py:123` / `test_xxx` / CI log | ○ / × / partial / n-a |
| 2 | ... | ... | ... |
...
### 推奨フロー
- **(A) 修正依頼が残っている場合**: `iterate-review <PR番号>` で review-fix ループを起動し、最終 summary コメントを 1 個投稿してマージ準備まで自動化
- **(A) 課題ゼロかつ受け入れ条件が全件 `○`**: ユーザーが `gh pr merge <PR番号> --squash` で squash merge → 紐づく issue は `close-issue <issue#>` でクローズ
...
4. Step 6 テンプレートの「受け入れ条件チェック」表に **判定列 `n-a` の 1 行**を置き、条件列に「該当なし (issue 未紐付け)」、実証列に代替判定根拠を書く (判定記号の正は §G.3 の 4 値 `○ / × / partial / n-a`。自由文と記号の 2 表現に割らない)
...
1. 代替として本 SKILL.md Step 3 + `enforce-acceptance-criteria/SKILL.md` の手順を手動で再現する (`gh pr view <PR番号> --json body` で PR 本文を取得し、受け入れ条件節を抽出 → 各条件を diff / test / log で実証)
...
1. `gh pr diff <PR番号> --name-only` で追加ファイルの存在を確認
