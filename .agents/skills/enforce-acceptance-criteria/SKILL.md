...
gh pr view <PR番号> --json body | python -c "import json,sys,re;b=json.load(sys.stdin)['body'];[print(m) for m in re.findall(r'Refs? #(\d+)|Fixes? #(\d+)', b)]"
...
```bash
gh pr comment <PR番号> --body "acceptance-criteria verified [<session-id>]
...
```bash
gh pr comment <PR番号> --body "修正依頼 (受け入れ条件未達) [<session-id>]
- 未達項目:
