...
ただし**過剰発火を避ける**: 検出対象は「**未解決の actionable な約束**」に限る。Step 6 トリアージで以下を確認し、該当すれば**残タスクなし**と判定して理由を記録してよい (闇雲な (B) 起票は noise issue を生む):

- 当該宣言が後続コメントで**解決済み / 撤回済み / superseded** になっている
