...
- 既知の制限 (post-match trailing 自動削除、[#805](https://github.com/Idios/kobutachan-allaganeye/issues/805)): `_drop_post_match_trailing` (#797) は試合終了後の trailing (lobby/city) を **scorebar 不在を根拠に不可逆削除**する。scorebar 検出 (`_has_scorebar_v2`) が FN する環境 (未対応 HUD layout / 4K Game DVR 等) では、scorebar を拾えない**実試合の trailing を silent に削除しうる** (試合 1 本喪失、エラーなし)。1080p OBS の 5 baseline では再現しないが、「不可逆削除 × 弱い否定信号」という構造的リスクとして残る。非破壊化 (削除 → フラグ方式) による恒久対策は #805 で追跡
