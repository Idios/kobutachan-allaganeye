...
**発火条件**: 本 PR が**新設した外部入力境界** (CLI option / metadata field / GUI 自由入力 / 環境変数) と、それが**到達する不可逆操作** (上書き / 削除 / truncate) がある場合。[`docs/l2-workflow.md` §Step 5 の focus 導出手順](docs/l2-workflow.md) と**同じトリガー語で発火する対の規約**であり、片方を実施したら他方も実施する。
...
**実装 PR では 4 問の答えを PR 本文に明示する。** 該当なしなら「新設した外部入力境界なし」を 1 行書く (無記載では「検査してゼロだった」と「検査しなかった」が区別できない)。
