"""probe 失敗縮退の統一契約型 (#824 spec sec.5.1)。
...
なるため独立配置 (#824 sec.5.1 module 配置)。
"""
...
class PresenceState(Enum):
    ...
    UNKNOWN -> ABSENT への折り畳みは集約層のみが明示的な state 比較で行う (sec.5.2)。
    """
...
class PresenceSample:
    ...
    property を持たない (#824 sec.5.1: silent bool 化経路の禁止)。
    """
...
class ProbeFailurePolicy(Enum):
    """集約層の probe 失敗方針 (#824 sec.5.2)。現時点の消費者は ISOLATE のみ。
