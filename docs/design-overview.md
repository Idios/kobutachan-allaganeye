...
> L7 (former L6) は暫定計画。詳細は `docs/release-process.md` を参照。
...
1. **Pass 1: 粗いスキャン** — 1-3s 間隔で全区間をサンプリング。動画をチャンクに分割し、チャンクごとに 1 プロセスの ffmpeg で dual seek + `select` filter デコードして暗転候補を収集 (正: [video-processing.md](video-processing.md) §フレームサンプリング方式)
...
**GPU モード** (`--gpu`): Pass 1 を GPU チャンク並列デコードで代替。CPU モードと同じ dual seek + `select` filter 方式だが、vendor 別 hwaccel によるハードウェアデコードを使い、チャンク数を動画長に応じて動的調整する (#437)。GPU 初期化コストを分散し高速化。Pass 1 以降は CPU/GPU 共通。利用不可時は CPU フォールバック。詳細は [video-processing.md](video-processing.md) §GPU アクセラレーション検知 が正。
...
外部ユーザーから受領した動画ファイルを処理する前に、独立ツール `kobutachan-allaganeye-guard` でセキュリティ検査を行う。**プログラムレベルでの結合は行わず**、主エージェント (Zed + DeepSeek) + 人間メンテナ Idios が手動で `allaganeye-guard verify` を実行する運用ルールとする (2026-04-21 方針確定、#454 参照)。
...
## ML モデル対応（将来: L5 (former L4)）

ローカル ML（scikit-learn 等）による投稿価値の評価を予定。具体的な技術選定は L5 (former L4) 着手時に再検討する。
