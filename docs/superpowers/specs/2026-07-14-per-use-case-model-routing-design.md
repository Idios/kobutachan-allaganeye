...
- **命名**: `allaganeye-{sonnet,haiku}-worker` はモデル名 + project prefix を焼き込む。将来ルーティング変更時の名前 churn の弱点は残るが、衝突回避と可視性を優先。`fable-consult` は user-level に同名が無いため prefix なし。
  - > **本条項は 2026-08-05 spec G1-2 / #945 で消化済み (2026-08-20)**。「user-level に同名が無い」という前提は**その後成立しなくなった** (`~/.claude/agents/fable-consult.md` が 2026-07-31 に作成された)。`allaganeye-fable-consult` へ改名済み。本行は当時の判断の historical record として残す。
...
- **retrospective 条項**: v0.3.0 リリース後（または導入から数週間後）に fable-consult / worker の実呼び出し実績を振り返り、形骸化していれば hook 強制化 or 廃止を再検討する。アドバイザリ運用（hook 見送り）の再評価点をここに固定する。
  - > **本条項は 2026-08-05 spec G1-2 / #945 で消化済み (2026-08-20)**。transcript 実測で実質レビュー 4 回の実績が確認されたため、形骸化していたのは**呼び出しではなく配線**だった。よって本条項が挙げる 2 択 (hook 強制化 / 廃止) のいずれでもなく **「skill step 化 + 申告義務」** を採った。本行は当時の再評価点の historical record として残す。
