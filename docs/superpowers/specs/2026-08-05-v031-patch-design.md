...
- **status**: Track 0 完了 / 08-31 に §9.4 を追記 (タグ打ちは 09-02 へずれ、§9.4 冒頭の注記参照)
- **作成日**: 2026-08-05
- **最終更新**: 2026-09-02 (§9.4 追記 08-31 + タグ打ちずれの注記。§9.1-9.3 は計画時点の記録として据え置く)
- **対象リリース**: v0.3.1 (patch)
...
本 table は `/release` Step 0c を実施した時点 (2026-08-05、本 spec 作成前) の open issue 集合が対象である。**§9.1-9.3 は計画時点の分類で、08-31 実施時点の再確認結果は §9.4 にある。**
...
タグ打ちは実機ゲート再実行 (#1015 / #1016) で 09-02 へずれ、当日は release workflow の修正 (#1026 / #1027) を経て 11:01 JST に打った。以下は 08-31 実施時点の記録。
...
Issue #1008 は本記録の後 (08-31 18:42 JST) に deferred 付与。49 件に含まれず、v0.3.2 計画時点の Step 0c で初回分類する。
...
- 走査 tip は `cc951ca`。それ以降にタグ tree へ載った PR (#1013 / #1015 / #1016 / #1026 / #1027) は範囲外。squash merge のため commit として区間に現れず、`cc951ca` は `refs/pull/1014/head` から辿れる (09-02 の事後走査で 0 件)
