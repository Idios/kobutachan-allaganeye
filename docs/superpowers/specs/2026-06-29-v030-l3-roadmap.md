...
- **minimap crop GUI 統合: #893** — MinimapScreen (drag-select + 数値 X/Y/W/H 入力 + 自動検出 + 切抜き実行 + 中断) + `start_minimap` Tauri command + `--json` / `--expected-mtime` CLI 拡張。AC: (1) spec 承認済、(2) overlay + crop + 進捗 GUI 完結、(3) minimap_regions 反映 + #514 CAS 整合、(4) CI + 実機検証、(5) roadmap v0.3.0 SSoT 追記（本行）
- #805 Phase 2 (GUI 視覚 UX)
- #818 (doc SSoT 規約 + release gate) — PR #855 で消化済み

**OUT (v0.4.0 期へ後送、2026-07-06 rescope)**:

- VTuber wiring: #809 (Pass 1 game 領域輝度適応) / #480 (ROI 適応分類 P4 のみ。P1 scorebar 局在化は PR #811 で merge 済)。追跡: #866 (two-signal Phase 3-4)。VTuber 対応再開時に gyawa (#481 本文のオファー) へ協力打診
...
> **2026-07-06 rescope** (#872): 旧計画で本 Phase 中核だった VTuber wiring (#809 ⟷ #480、audit P1-8 で coupling) は v0.4.0 期へ後送 (追跡: #866)。v0.3.0 に残るのは #810 (capture region 基盤) / #822 / #824 (masked 品質) で、minimap (#481) を**最初に再開する workstream** として組み込む (決定文言「まず minimap 切抜きとチャット欄画像マスク対応を先行」)。

```text
#810 (metadata capture領域 schema) ──基盤──┬─▶ #481 (minimap 切抜き)  ← 最初に再開する workstream (un-defer)
                                            │
                                            └─▶ #822 (localize 分類器精緻化 = masked 過分割解消)
                                                  ▼
                                                #824 (probe 失敗縮退 semantics 統一契約)  ← 診断基盤、再アーキ Phase 2 と同一実装期に確定 (2026-07-03 spec)
```

- 着手順序: #810 完成後は **#481 (minimap) を最初に再開**し、masked 品質 track (#822 → #824) は #481 と並行可の follow-up として進める。
- dispatch: 設計重め (#824 / #481) は brainstorming → writing-plans → subagent-driven-development → /review-pr + Codex adversarial-review (tier 1 = `codex-companion.mjs adversarial-review`、#795)。
...
- **#824**: 設計 spec 先行 (brainstorming → spec)。detection-map §5.5 (旧 §5.4) Phase 3 cutover と統合するか独立かは着手時判断。→ [2026-07-03 設計 spec](2026-07-03-issue-824-probe-failure-semantics-design.md) で確定: 再アーキ spec Phase 2 (Stage 2 統合) と同一実装期、Phase 4 cutover とは独立 (旧「Phase 3 cutover」呼称は supersede 済み、spec §3 参照)。
...
### 5.1 ラベル整合 (2026-07-06 rescope、PR #873 merge 後に実施)
...
経緯 comment (v0.4.0 送りの判断根拠 + gyawa 協力打診トリガー) は PR #873 merge 後に #809 / #480 / #753 / #866 へ記録する。実施記録は #872 を参照。
...
- 2026-07-06 rescope: #872 (doc・ラベル同期) / release gate 差し替えは PR #855 (commit `c1b0c82`) で消化済み
