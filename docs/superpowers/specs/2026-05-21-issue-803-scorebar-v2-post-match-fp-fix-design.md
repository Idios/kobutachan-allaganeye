...
| Field | Value |
| --- | --- |
| **Status** | #803 gate + #797 M7 drop は 5-baseline verified (2026-05-22)。#797 trailing-drop は Codex 再 review 3 回を受けて (C') midpoint → (C'') multi-probe → (C''') early-window scan → (C'''') window-end probe 追加 に強化 (2026-05-23/24、§0.6「実装の変遷」)。(C'') は 5-baseline 実機再検証済 (27 match / 54 agreed / 0 finding)。round-4 で baseline split.json の systemic staleness ([medium]) + その root cause の `generate-v030-baselines.py` bug (絶対 tmp path / `--gpu` 欠落) も判明し、Idios 判断で本 PR 内で script fix + 5 baseline regen を同梱。(C'''') + 整合 baseline は 5-baseline 実機再検証済 (27 match / 54 boundary 全 agreed / 0 finding、matches byte 同一)。さらに round-5 で whole-video unknown フォールバックを誤 drop しうる fail-open regression ([high]) に対し gate を追加したが、round-6 でその gate の `segments[-2]` type 判定が single-match 録画 ([match(unknown), post-match(unknown)]) の post-match drop を block し #797 FP を再発させると判明 ([high])。gate を `len(segments) < 2` のみ (whole-video fail-open のみ保護、他は scorebar probe に委譲 = round-4 挙動への収束) に単純化。gate は 5 baseline に output-neutral (obs-20260116 は len>=2 で probe → M7 drop 維持) のため baseline 再生成不要。さらに PR 作成 Pre-flight Step 5 の Codex adversarial-review で **#803 gate 側**に [medium] (Rescue path の `_find_scorebar_horizontal_range` が **longest run** を選んでから gate するため、より長い off-center / over-wide band が有効な中央 scorebar run を mask して frame 全体を false-negative にしうる、特に Rescue が支える 4K / Game DVR layout #522) を検出。run 選択を **中央跨ぎ (x=960 含有) run の直接選択** に補正 (commit 22a7f63、§0.4.1)。本 fix も 5 baseline output-neutral (obs-20260116 確認済 + 他 4 を Pre-flight で再検証、§3.3) |
...
### §0.4.1 Step-5 codex 補正: run 選択を中央跨ぎに (2026-05-26)

PR 作成 Pre-flight Step 5 の `/codex:adversarial-review` で、上記 width / 中央 gate を `_find_scorebar_horizontal_range` の **longest run** (`max(merged, key=width)`) に適用していた実装が、より長い off-center / over-wide band (UI widget / 派手な内装) が**有効な中央 scorebar run を mask** して frame 全体を false-negative にしうる ([medium]、特に Rescue path が支える 4K / Game DVR layout #522) と判明。

修正 (commit 22a7f63): longest 選択を廃し、**画面中央 (x = `_SCOREBAR_V2_PROBE_WIDTH // 2` = 960) を含む run を直接選択** (`center_run = next(run for run in merged if run[0] <= 960 <= run[1])`、runs は disjoint なので中央含有は高々 1 個) → width 下限 / 上限 gate、の順に変更。中央を含む run が無ければ `None` (edge-confined band を reject)。これにより「longest が gate を通っても、それが中央 scorebar でない」ケースで中央 scorebar を取りこぼさない。gate 自体の意味 (width 上限 + 中央位置) は §0.4 のまま、**選択順序のみ補正**。5 baseline 実機 re-verify で output-neutral を確認 (§3.3)。unit test は §4.1 に `test_longer_offcenter_run_does_not_mask_centered_scorebar` を追加。
...
**実装の変遷 (#797 trailing-drop、Codex review chain)**: 上記 **対策 A** (最後の有効 blackout が `match_boundary` なら drop) は無方向な `match_boundary` 分類を key にしていたため、「lobby→試合開始→EOF まで試合」録画で真の試合 trailing を誤 drop しうる、と Codex adversarial-review (2026-05-22) が [high] 指摘。これを受けて専用 helper `_drop_post_match_trailing` が **trailing segment 自体の scorebar を probe** する方式 (C') に置換 (commit 64a72ba / d759fe9)。さらに再 review (2026-05-23) で「単一 midpoint probe は *mixed* trailing (真の試合 + より長い post-match tail。試合終了 blackout が `non_fl` 誤分類等で scorebar.py で drop された場合に発生) の試合部分まで誤 drop しうる」と [high] 指摘され、early (`_TRAILING_PROBE_START_OFFSET` で開幕 blackout を越えた点) / midpoint / late の **multi-probe guard** に強化。全 probe が definite miss (`False`) のときのみ drop し、1 つでも scorebar hit (`True`) / probe 失敗 (`None`) なら保持する (safe side)。この (C'') multi-probe 版は実機 audit-compare で 5-baseline 検証済 (27 match / 54 boundary 全 agreed / 0 finding、matches は committed と byte 同一)。さらに 3 度目の adversarial-review (2026-05-23) で「固定 12s の early probe は long-loading で HUD が遅延すると loading 画面に落ち、midpoint/late が長い post-match に落ちると mixed trailing の試合を誤 drop しうる (`min_match_duration=300` 前提で 5-8 分の最終試合 + 長い city tail は設定上ありうる)」と [high] 指摘。これを受け (C''') で early probe 単点を **early-window scan** に置換: trailing 先頭の candidate-match window (`start` .. `start + min_match_duration`、segment に clamp) を `_TRAILING_PROBE_STRIDE` (=60s) 間隔で走査し、全 probe が definite miss のときのみ drop (1 つでも hit / None なら保持)。早期 HUD 遅延を strided window で吸収するため固定 offset の死角が消える。さらに 4 度目の adversarial-review (2026-05-24) で 2 finding: **[high]** early-window scan が `timestamp < window_end` で window 末尾の stride gap (`start+240`..`start+300`) を未 probe にし、loading が window 末尾近くまで続く場合に試合 HUD を取りこぼす。**[medium]** 再生成すると baseline の split.json が committed (05-18/19) と metadata (05-22) で不整合 = 全 5 baseline で stale。**[high] 対策**: strided probe に加え window 末尾 (`min(window_end, end-1.0)` で EOF read を回避) を必ず probe する点を追加し、stride gap を残さない。**[medium] root cause**: `generate-v030-baselines.py` が (a) split 後の metadata を再読込して `output_file` に絶対 tmp path を焼き込み、(b) `detect` を `--gpu` 無しで呼び `use_gpu/gpu_vendor_used=null` を記録していた。fix: detect を `--gpu` で呼び、split 前の detect metadata (相対 placeholder) を payload に使う。Idios 判断で本 PR 内で 5 baseline を fixed script で regen し metadata+split を整合させる (`tests/scripts/test_generate_v030_baselines.py` に `output_file` 相対性の committed-fixture guard を追加)。probe 位置がまた変わるため 5-baseline を再検証し、27 match / 54 boundary 全 agreed / 0 finding (matches byte 同一、obs-20260116 M7 drop 維持) を確認。さらに 5 度目の adversarial-review (2026-05-24) で **[high]** 「`_drop_post_match_trailing` は動画全体 1 つの unknown フォールバック (blackout 不在時の fail-open) も drop しうる → 0 match / DetectionError」と指摘。対策: drop を「`len(segments) >= 2` かつ直前 segment が非 `unknown` (確定 match の trailing)」のときに限定する gate を追加。lone whole-video unknown / 直前も unknown のケースは保持 (fail-open)。gate は 5 baseline に output-neutral (obs-20260116 の直前 M6=`fl_match` で通過 → M7 drop 維持、他 4 は trailing 不在で未到達) のため baseline 再生成は不要、unit test で検証。さらに 6 度目の adversarial-review (2026-05-24) で **[high]** 「round-5 gate の `segments[-2]["type"] == "unknown"` 条件は、`_filter_and_extract_segments` が before-first / after-last segment を `unknown` で hardcode するため、single-match 録画 [match(unknown) → warp → post-match(unknown)] で post-match drop を block → #797 FP を single-match 構成で再発させる」と指摘。対策: gate を **`len(segments) < 2` のみ** (= lone whole-video fallback だけ保護、それ以外は既存 scorebar probe に委譲) に単純化。実 match の tail は probe が scorebar で守るため single match + 無 scorebar post-match tail は正しく drop され、round-4 の検証済み挙動 + whole-video fail-open に収束。gate は 5 baseline に output-neutral (obs-20260116 は len>=2 で probe → M7 drop 維持) のため再生成不要、unit test (whole-video unknown → keep / single-match post-match tail → drop) で検証 (§3.3 / §4.4、Iron Law 6)。
