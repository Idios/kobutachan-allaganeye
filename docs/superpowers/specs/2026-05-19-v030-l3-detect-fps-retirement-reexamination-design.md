...
## 10. V6.2 (#797) — scorebar HUD binary search for open-ended unknown matches

§9 で audit-first direction を採用後、PR #793 baseline を audit-compare で検証した
結果 (`docs/v030-baseline-audit.md` §"2026-05-21 PR #793 verification update")、
F1 / F4 は PR #793 で fix 確認、obs-20260118 split が real boundary であることも確定。
**唯一の残課題が #797 (obs-20260116 M6 end miss)** で、user 選択 scope (D) に従い
PR #793 内で fix する。

### 10.1 #797 の構造 (V2 root cause analysis 結果)

obs-20260116 M6 end (GT 6540):

- Real M6 end は Fanfare / VICTORY moment ≈ 6540 (Idios 視覚確認)
- t=6540 周辺の brightness は 95+ (no blackout signal)
- 最寄りの blackout は t=6900 (3.0、single 12s sample) で 360s 後の GC scoreboard 遷移
- `_filter_and_extract_segments` の "After last blackout" path で M6 が
  `{start: 5624.25, end: total_duration=7303.488, type: "unknown"}` として
  video 末尾まで延長

つまり「**match end が blackout を伴わず Fanfare / VICTORY のみ**」の case を、
従来の brightness-only 検出が捕捉できないことが構造的限界。

### 10.2 V6.2 アプローチ: scorebar HUD binary search

User suggestion (2026-05-21): 全フレーム scan ではなく**二分探索で境界位置を絞る**。

**前提**: in-match scorebar HUD は match 中に visible、match 終了後 (GC scoreboard /
lobby) は absent。両端で確証を取れば、二分探索で transition 点を log2(N) probe で
発見可能。

**Algorithm**:

```text
1. Skip if match duration < min_match_for_search (default 120s)
2. Probe scorebar at start_probe = match_start + 60s
   - If not True (absent / inconclusive) -> return None (unusual match start)
3. Probe scorebar at end_probe = match_end_upper - 10s
   - If not False (present / inconclusive) -> return None (truncated recording)
4. Binary search [start_probe, end_probe]:
   - mid = (lo + hi) / 2
   - If scorebar present -> lo = mid
   - If scorebar absent -> hi = mid
   - If None -> return None (conservative)
5. Loop until hi - lo <= resolution_seconds (default 1.0)
6. Return hi (first absent timestamp)
```

**Scorebar detection**:

- Primary: `_has_scorebar_v2` (1920x1080 + GC emblem 3-point AND、#307 / #522)
- Fallback: `_has_scorebar` (channel-std + edge、V2 が None の場合)

**Promotion**: 検出した transition timestamp を match end に上書き、type を
`unknown -> fl_match` に変更、`DetectionStats.scorebar_search_promotions` をインクリメント。

### 10.3 Performance estimate

obs-20260116 M6 (range = 5624-7303 = 1679s) で resolution_seconds=1.0 の場合:

- log2(1679 / 1.0) = ~10.7 -> ~11 binary search probes
- 各 probe = ffmpeg frame extract + scorebar V2 detect ≈ 0.5-1s
- 合計: **~10-15s 程度** per open-ended unknown match
- 通常 1 recording で 0-1 件 (M6 end のみ) なので影響極小

### 10.4 Risk と mitigation

| Risk | Mitigation |
| --- | --- |
| scorebar が match 中に一時的に消失 (camera angle / map screen) | binary search が monotonic でない -> mid probe で False になると hi が誤った位置に収束。実害は M6 end が早すぎる方向にずれる (still type=fl_match)。`tolerance_sec=5s` で吸収可能 |
| scorebar V2 が opencv 不在で動かない | V1 fallback あり、両方 None なら return None (conservative) |
| 非 FL recording (VTuber 等 #480) で scorebar 不在 | start_probe で None / False -> return None、match は unknown のまま (safe) |
| 開始 60s 後にも scorebar 未表示 (intro cinematic 長い) | start_probe で False -> return None、match は unknown のまま |
| 終了直前に scorebar 再表示 (post-match recap) | end_probe で True -> return None、match は unknown のまま (false positive 回避) |

### 10.5 実装位置と integration

- 関数: `_check_scorebar_present_at`, `_find_match_end_via_scorebar`,
  `_refine_open_ended_unknown_matches` を `allaganeye/video/detector.py` の
  `_padded_start` 以降に追加。
- 呼び出し: `detect_match_boundaries` の `_filter_and_extract_segments` 後 (line 411 付近)。
  `src_resolution is not None` (scorebar 検出有効化条件) を継承して post-process。
- Stats: `DetectionStats` TypedDict に `scorebar_search_promotions: int` を追加。
- GPU detector への影響: 無し (gpu_detector.py は brightness のみ返し、segment 抽出は
  detector.py 経由なので V6.2 が自動適用される)。

### 10.6 Test (TDD red -> green)

`tests/test_detector.py`:

- `TestFindMatchEndViaScorebar` (7 tests): 短すぎ match / start probe 失敗 / end probe 失敗 /
  start probe None / 二分探索収束 / mid probe None / resolution_seconds 効果
- `TestRefineOpenEndedUnknownMatches` (6 tests): open-ended unknown 昇格 /
  fl_match 不変 / non-open-ended 不変 / search None で不変 / stats populate /
  obs-20260116 シナリオ end-to-end mock

全 13 tests PASS (2026-05-21 commit 時点)。`pytest tests/ -q` で 1240 passed / 13 skipped、
regression 無し。`ruff check / format`、`pyright` も pass。

### 10.7 実機検証 (Idios タスク)

V6.2 mock test pass だけでは scorebar 検出の実機挙動を保証できない (mock 不可)。
Iron Law 6「mock test pass = 実機検証不要」は Red Flag のため、Idios が以下を実施:

1. **V6 verify**: `allaganeye detect "<sample_dir>/20260116/2026-01-16 22-12-57.mkv"` 再実行
   - 期待: M6 end が 6540 ±5s 内、type=fl_match
   - 期待: verbose log に `Open-ended unknown match refined via scorebar binary search`
2. **V3**: 他 3 baseline を PR #793 detector で regenerate
   - `allaganeye detect "<sample_dir>/20260119/2026-01-19 22-09-07.mkv"`
   - `allaganeye detect "<sample_dir>/20260127/2026-01-27 21-59-15.mkv"`
   - `allaganeye detect "<sample_dir>/2026-02-09 23-12-24.mkv"` (obs-20260209)
   - 各 metadata.json を `tests/baselines/v0.3.0/obs-*.metadata.json` に上書き
   - Claude が audit-compare 実行、新 false positive / silent miss 有無を確認
3. **Audit-compare 再実行 (5 baseline)**:
   - 各 baseline の findings が 0 になっていることを確認
   - obs-20260116 M6 end が refined されている (boundary_shift -> agreed) ことを確認

### 10.8 失敗時の fallback

V6 verify で M6 end が ±5s 内に収束しない / scorebar 検出が動かない場合:

- (1) `_check_scorebar_present_at` の挙動を obs-20260116 ~5800 (mid-M6) と
  ~6540 (transition) と ~7250 (post-match) で実機確認、V2 / V1 のどちらが
  動いているか / None が返るか調査
- (2) 必要なら start_probe_offset / end_probe_margin を調整 (e.g., 60s -> 120s)
- (3) それでも収束しない場合は **#797 を v0.3.x に defer**、PR #793 は obs-20260116 M6 を
  unknown のまま ship (V6.2 は他 recording の同種 case に活用される infra として残る)
