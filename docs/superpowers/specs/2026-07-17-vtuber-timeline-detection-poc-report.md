...
### 7.4 振り返りパート (replay/review シーン) の挙動 (2026-07-17 Idios 指摘で検証)

きゅま VOD には試合後に「部屋シーン」(別 OBS シーン、ウィンドウ内でリザルト表や試合 footage を再生する振り返りパート) が挿入されることがある。本 VOD では **t=5710-5980 (M5 と M6 の間、~4.5min)** に存在することをフレーム目視で確認 (5750=リザルト表 / 5850=町 footage / 5950=戦闘 footage 再生中)。

timeline 計測での挙動 (10s stride、27 probes):

| 区間の実態 | present | band_mad | evidence (present∧mad≥1.5) |
| --- | --- | --- | --- |
| 5710-5790: 静止リザルト表がウィンドウ内表示 | 100% (バーが anchor に重なる) | **0.27-0.70 (凍結)** | 0 |
| 5800-5910: 町 footage / シーン内静止 | 0% | 0.3-1.7 | 0 |
| 5920-5950: 戦闘 footage 再生中 | **0% (縮小+オフセットでバーが anchor を外れる)** | 1.6-2.2 (動く) | 0 |

→ **振り返り区間全体で match evidence = 0/27。segmentation は 4730-5710 / 5980-6600 と正しく分割し、振り返りを非試合として除外した。** 防御は設計どおり 2 機構が独立に効いた: ①静止表示は「present だが frozen」で motion AND が棄却、②再生中 footage は「動くが、縮小ウィンドウ内のバーが位置厳格な at-anchor を外れる」で presence が棄却。

残存リスク: 振り返りが**ほぼ 1:1 スケール・無停止・300s 以上**連続再生された場合は原理的に騙されうる (design spec リスク R-h として明記)。なお 9340-9710 の短 segment は疑ったが、フレーム目視でライブ試合レイアウト (アバター+コメント欄+マップ) と確認し試合で確定。
