...
- **CHANGELOG 見出し日付 = タグを打つ日 (JST)** — `CHANGELOG.md` の `## [x.y.z] - YYYY-MM-DD` の日付は、そのバージョンの**タグを打つ日**を `Asia/Tokyo` で表した値とする (裁定 D6、#948)
  ...
  - タグを打つ直前に見出し日付を当日へ更新して commit する。手順は [`.claude/skills/release/SKILL.md`](../.claude/skills/release/SKILL.md) §タグ打ち・GitHub Release 作成
  - 機械検査は [`scripts/check_version_consistency.py`](../scripts/check_version_consistency.py) の `--tag` 指定時のみ発火する。基準日は annotated tag の `taggerdate`、取得できない場合 (lightweight tag 等) は push event の `head_commit.timestamp` を `--changelog-date-from` で渡す。**検査対象外の集合**は同スクリプトの `_check_changelog_heading()` docstring に列挙してある
