...
試合中の UI 要素（スコアボード、ミニマップ、キルログ等）の位置や見た目を把握するための資料。L1（暗転検知）・L3 (new) (VTuber UI 適応、2026-07-17 再開 #895)・L4 (former L3, OCR) の設計に直接役立つ。
...
> プロジェクトのサンプルデータは環境変数 `ALLAGANEYE_SAMPLE_VIDEO_DIR` で指定したディレクトリの自前の録画を使用する（詳細は `AGENTS.md` 参照）。
