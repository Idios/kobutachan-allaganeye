...
| `--name-pattern PATTERN` | `{idx:03}_{type}_{start}_minimap.mp4` | 出力ファイル名テンプレート。使用可能トークン: `{idx}` / `{idx:03}` / `{type}` / `{start}` (MM-SS 形式。1 時間以上の場合は H-MM-SS) / `{date}`。**拒否条件は export の同名オプションと同一** ((a) `-o` の外 / `-o` 自身 (b) 元動画 (`source`) (c) 2 つ以上の match が同一ファイルに解決 (d) Windows の `:` (e) 解決不能パス)。**判定は metadata write-back / mkdir / ffmpeg のいずれよりも前**に行うため、拒否時は `metadata.json` も更新されない。`-o` 省略時の基準は `<metadata dir>/minimap/` (#930 / #938) |
