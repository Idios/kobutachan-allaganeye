...
| `--name-pattern PATTERN` | `{idx:03}_{type}_{start}.mp4` | 出力ファイル名テンプレート。使用可能トークン: `{idx}` / `{idx:03}` / `{type}` / `{start}` (MM-SS 形式。1 時間以上の場合は H-MM-SS、例: `1-23-41`) / `{date}`。展開結果が `--output-dir` の外に出る場合、および **2 つ以上の match が同一ファイルに解決される場合** は 1 件も書き出さずに exit 5 (後者は `{idx}` / `{idx:03}` を含めれば回避できる。`{type}` は `metadata.json` の値をそのまま使うため、文字列としては異なるのに同じファイルを指す名前 — Windows が開くときに無視する大文字小文字違い・末尾のドット/空白、および `..` を含むパス — になりうる) |
