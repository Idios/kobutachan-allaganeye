...
| store mutation | debounce 完了時 `metadataStore.metadata.matches[i].edited.start_time/end_time`、`dirty=true`。brightness overlay は store mutation なし (PreviewScreen 内 local state、`brightnessWindow` / `microError`) |
