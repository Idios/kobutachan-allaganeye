...
| 遷移トリガー | `onClick` → `handleStartExport()` → `dispatch(START_CLICKED)` (idle→running) → 単発 `invoke('start_export', ...)` → Python subprocess が N 並列 ffmpeg を spawn → 各試合の進捗は `export-progress` イベントで per-match state を直接更新 (reducer は経由しない) → 全完了で [export.ts](../gui/src/screens/reducers/export.ts) の `PROGRESS_COMPLETE` / 全失敗で `EXPORT_ERROR` / cancel で `CANCEL_CONFIRMED` |
