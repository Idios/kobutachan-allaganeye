...
外部から受領したファイルを処理する前に、エージェント (Zed + DeepSeek + 人間メンテナ Idios の両方) が手動で検査する。
...
3. **エージェント (Zed + DeepSeek + Idios) が `allaganeye-guard verify` を実行**。PASS するまで allaganeye で処理しない
