...
class TestFlagPostMatchTrailing:
    ...
    @patch("allaganeye.video.detector._has_scorebar_v2", return_value=False)
    @patch("allaganeye.video.detector._probe_frame_rgb_hires", return_value=b"x")
    def test_trailing_flag_invokes_callback_once(self, _probe, _v2):
        """On a post-match decision, trailing_drop_callback fires once with (start, end).

        The callback stays wired in #805 段階2 Task 3 (removed in Task 4); the
        segment is now retained (flagged) rather than dropped.
        """
        segments = [
            {"start": 100.0, "end": 1000.0, "type": "fl_match"},
            {"start": 1000.0, "end": 1800.0, "type": "unknown"},
    ...
    @patch("allaganeye.video.detector._has_scorebar_v2", return_value=False)
    @patch("allaganeye.video.detector._probe_frame_rgb_hires", return_value=b"x")
    def test_trailing_flag_invokes_callback_with_stats_none(self, _probe, _v2):
        """On a post-match decision the callback fires even when ``stats is None``.

        The callback path sits intentionally outside the ``if stats is not
        None:`` block, so non-verbose runs (stats=None) still fire it. Pin that
        the seam is independent of stats collection (callback removed in Task 4).
        """
        segments = [
            {"start": 100.0, "end": 1000.0, "type": "fl_match"},
            {"start": 1000.0, "end": 1800.0, "type": "unknown"},
    ...
    def test_trailing_keep_does_not_invoke_callback(self, _probe, _v2):
        ...
        assert len(result) == 2
        # No post-match decision reached -> no flag set.
