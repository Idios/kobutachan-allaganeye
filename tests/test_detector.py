...
from allaganeye.video.detector import (
    ...
    _refine_open_ended_unknown_matches,
...
class TestFindMatchEndViaScorebar:
    """Tests for V6.2 scorebar binary search.

    Targets #797 obs-20260116 M6 end miss: a match that ends without a
    visible blackout signal (Fanfare / VICTORY moment) so the detector
    falls back to total_duration with type=unknown. The binary search
    refines the open-ended end via scorebar HUD transition.
    """

    def test_returns_none_when_match_too_short(self):
        """Match shorter than min_match_for_search returns None."""
        result = _find_match_end_via_scorebar(
            Path("/dummy.mp4"),
            match_start=100.0,
            match_end_upper=150.0,
            height=1080,
            min_match_for_search=120.0,
        )
        assert result is None

    def test_returns_none_when_no_scorebar_at_start(self, monkeypatch):
        """Returns None when scorebar absent at start probe (unusual match start)."""
        monkeypatch.setattr(
            "allaganeye.video.detector._check_scorebar_present_at",
            lambda video_path, t, height: False,
        )
        result = _find_match_end_via_scorebar(
            Path("/dummy.mp4"),
            match_start=5624.0,
            match_end_upper=7303.0,
            height=1080,
        )
        assert result is None

    def test_returns_none_when_scorebar_present_at_end(self, monkeypatch):
        """Returns None when scorebar still present near end (match truncated)."""
        monkeypatch.setattr(
            "allaganeye.video.detector._check_scorebar_present_at",
            lambda video_path, t, height: True,
        )
        result = _find_match_end_via_scorebar(
            Path("/dummy.mp4"),
            match_start=5624.0,
    ...
    def test_returns_none_when_start_probe_inconclusive(self, monkeypatch):
        """Returns None when start probe yields None (frame extraction failed)."""
        monkeypatch.setattr(
            "allaganeye.video.detector._check_scorebar_present_at",
            lambda video_path, t, height: None,
        )
        result = _find_match_end_via_scorebar(
            Path("/dummy.mp4"),
            match_start=5624.0,
    ...
    def test_binary_search_converges_to_transition(self, monkeypatch):
        """Binary search converges to scorebar present->absent transition."""
        transition = 6540.0

        def mock_scorebar_present(video_path, t, height):
            return t < transition

        monkeypatch.setattr(
            "allaganeye.video.detector._check_scorebar_present_at",
            mock_scorebar_present,
        )
        ...
        # Result is the first "absent" timestamp; should be within resolution
        # of the true transition.
        assert abs(result - transition) <= 1.0

    def test_binary_search_returns_none_when_mid_probe_inconclusive(self, monkeypatch):
        """Returns None when scorebar detection is inconclusive (None) mid-search."""
        # Present at start probe, absent at end probe, but inconclusive in
        # between.  Conservative: return None rather than guess.
        start_probe = 5624.0 + 60.0  # match_start + default offset
        end_probe = 7303.0 - 10.0  # match_end_upper - default margin
        ...
        def mock_scorebar_present(video_path, t, height):
            ...
            if t == end_probe:
                return False
            return None  # inconclusive mid-search
    ...
    def test_resolution_seconds_respected(self, monkeypatch):
        """Higher resolution_seconds yields wider convergence band."""
        transition = 6540.0
        ...
        result_fine = _find_match_end_via_scorebar(
            Path("/dummy.mp4"),
            match_start=5624.0,
            match_end_upper=7303.0,
            height=1080,
        ...
        result_coarse = _find_match_end_via_scorebar(
            Path("/dummy.mp4"),
            match_start=5624.0,
            match_end_upper=7303.0,
            height=1080,
        ...
        # Both converge near transition; fine is closer
        assert abs(result_fine - transition) <= 0.5
        assert abs(result_coarse - transition) <= 5.0


class TestRefineOpenEndedUnknownMatches:
    """Tests for _refine_open_ended_unknown_matches (V6.2 post-processing)."""

    def test_open_ended_unknown_match_refined(self, monkeypatch):
        """Match with end==total_duration and type=unknown gets refined."""

        def mock_search(video_path, match_start, match_end_upper, height, **kwargs):
            return 6540.5

        monkeypatch.setattr(
            "allaganeye.video.detector._find_match_end_via_scorebar",
            mock_search,
        )

        matches: list[MatchBoundary] = [
            {"start": 5624.0, "end": 7303.488, "type": "unknown"},
        ]
        result = _refine_open_ended_unknown_matches(
            matches,
            Path("/dummy.mp4"),
            height=1080,
            total_duration=7303.488,
        )

        assert len(result) == 1
        assert result[0]["end"] == 6540.5
        assert result[0]["type"] == "fl_match"

    def test_fl_match_not_touched(self, monkeypatch):
        """Matches already classified as fl_match are not refined."""
        call_count = {"n": 0}
        ...
        result = _refine_open_ended_unknown_matches(
            matches, Path("/dummy.mp4"), height=1080, total_duration=2000.0
        )

        assert call_count["n"] == 0  # search not invoked
        assert result[0]["end"] == 1000.0
        assert result[0]["type"] == "fl_match"

    def test_unknown_match_not_at_video_end_not_touched(self, monkeypatch):
        """Match with type=unknown but end well before total_duration not refined."""
        call_count = {"n": 0}
        ...
        matches: list[MatchBoundary] = [
            {"start": 0.0, "end": 1000.0, "type": "unknown"},
        ]
        result = _refine_open_ended_unknown_matches(
            matches, Path("/dummy.mp4"), height=1080, total_duration=2000.0
        )

        assert call_count["n"] == 0  # search not invoked (end < total_duration - 1)
        assert result[0]["end"] == 1000.0
        assert result[0]["type"] == "unknown"

    def test_search_returns_none_match_unchanged(self, monkeypatch):
        """When search returns None, match stays open-ended unknown."""
        monkeypatch.setattr(
            "allaganeye.video.detector._find_match_end_via_scorebar",
            lambda *a, **kw: None,
        )
        ...
        result = _refine_open_ended_unknown_matches(
            matches, Path("/dummy.mp4"), height=1080, total_duration=7303.488
        )

        assert result[0]["end"] == 7303.488
        assert result[0]["type"] == "unknown"

    def test_stats_populated_on_promotion(self, monkeypatch):
        """stats['scorebar_search_promotions'] increments on each promotion."""
        monkeypatch.setattr(
            "allaganeye.video.detector._find_match_end_via_scorebar",
            lambda *a, **kw: 6540.0,
        )
        ...
        assert stats.get("scorebar_search_promotions") == 1

    def test_obs_20260116_m6_end_scenario(self, monkeypatch):
        """End-to-end obs-20260116 M6 end refinement scenario (#797).

        Simulates the obs-20260116 layout: 5 fl_match boundaries + 1
        open-ended M6 at the end. Only M6 is refined; others untouched.
        """

        def mock_search(video_path, match_start, match_end_upper, height, **kwargs):
            # Only obs-20260116 M6 hits this in real usage
            if match_start == 5624.25 and match_end_upper == 7303.488:
                return 6540.0
            return None
        ...
        matches: list[MatchBoundary] = [
            {"start": 49.125, "end": 1054.5, "type": "fl_match"},
            {"start": 1256.0, "end": 2178.75, "type": "fl_match"},
            {"start": 2355.0, "end": 3230.5, "type": "fl_match"},
            {"start": 3365.0, "end": 4352.0, "type": "fl_match"},
            {"start": 4538.0, "end": 5482.5, "type": "fl_match"},
            {"start": 5624.25, "end": 7303.488, "type": "unknown"},
        ]
        result = _refine_open_ended_unknown_matches(
            matches, Path("/dummy.mp4"), height=1080, total_duration=7303.488
        )
        ...
        # First 5 unchanged
        for i in range(5):
            assert result[i]["type"] == "fl_match"
            assert result[i]["end"] == matches[i]["end"]
        # M6 refined
        assert result[5]["start"] == 5624.25
        assert result[5]["end"] == 6540.0
        assert result[5]["type"] == "fl_match"
