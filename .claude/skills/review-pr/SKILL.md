...
7. **CI ステータス確認**: `gh pr checks $ARGUMENTS` を実行し、全チェックの結果を確認する
   - 失敗しているチェックがある場合: マージ不可。失敗しているジョブ名と概要をユーザーに報告し、PR 作成者のロールに `role:*` ラベルを付替えて差し戻す
   - チェック未完了の場合: 完了を待つか、時間がかかりそうならユーザーに判断を仰ぐ
   - 全チェック pass: マージ可として次に進む
   - CI が設定されていないリポジトリではこのステップをスキップする
8. レビュー結果をユーザーに報告し、判断を仰ぐ:
